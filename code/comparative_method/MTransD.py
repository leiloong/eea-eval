...
def structure_embedding(folder, radio):
    ...
    with graph.as_default():
        ...
        with tf.variable_scope('relation2vec' + 'embedding'):
            ...
            ent_transfer = tf.get_variable(name="ent_transfer", shape=[ent_num, embed_size],
                                                initializer=tf.contrib.layers.xavier_initializer(uniform=False))
            rel_transfer = tf.get_variable(name="rel_transfer", shape=[rel_num, embed_size],
                                                initializer=tf.contrib.layers.xavier_initializer(uniform=False))
        ...
        optimizer, loss = transe_loss(pos_h_e, pos_r_e, pos_t_e, neg_h_e, neg_r_e, neg_t_e, margin)
        ...
        with tf.Session(graph=graph, config=config) as sess:
            ...
            for epoch in range(1, epochs+1):
                ...
                for step in range(num_steps):
                    ...
                    feed_dict = {pos_h: [x[0] for x in batch_pos],
