...
def structure_embedding(folder, radio):
    res_folder = generate_res_folder(folder, "mtransr", radio)
    ...
    with graph.as_default():
        ...
        with tf.variable_scope('relation2vec' + 'embedding'):
            ...
            rel_matrix = tf.get_variable(name="rel_mat", shape=[rel_num, embed_size * embed_size],
                                              initializer=tf.contrib.layers.xavier_initializer(uniform=False))
                        
            ref_ent_s = tf.constant(ref_ent1, dtype=tf.int32)
            ref_ent_t = tf.constant(ref_ent2, dtype=tf.int32)

        phs = tf.reshape(tf.nn.embedding_lookup(ent_embeddings, pos_hs), [-1, embed_size, 1])
        prs = tf.nn.embedding_lookup(rel_embeddings, pos_rs)
        pts = tf.reshape(tf.nn.embedding_lookup(ent_embeddings, pos_ts), [-1, embed_size, 1])
        nhs = tf.reshape(tf.nn.embedding_lookup(ent_embeddings, neg_hs), [-1, embed_size, 1])
        nrs = tf.nn.embedding_lookup(rel_embeddings, neg_rs)
        nts = tf.reshape(tf.nn.embedding_lookup(ent_embeddings, neg_ts), [-1, embed_size, 1])
        ...
        phs = tf.reshape(tf.matmul(p_matrix, phs), [-1, embed_size])
        pts = tf.reshape(tf.matmul(p_matrix, pts), [-1, embed_size])

        # phs = tf.nn.l2_normalize(phs, 1)
        # pts = tf.nn.l2_normalize(pts, 1)

        nhs = tf.reshape(tf.matmul(n_matrix, nhs), [-1, embed_size])
        nts = tf.reshape(tf.matmul(n_matrix, nts), [-1, embed_size])
